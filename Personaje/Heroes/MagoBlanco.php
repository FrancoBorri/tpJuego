...
class MagoBlanco extends Hechicero
{
    ...
    public function __construct(
        $nombre,
        int $mana = 10,
        int $nivel_de_maestría = 10,
        $desplazamiento = 1,
        $ataque = 3,
        $vida = 5,
        $habilidades = ["combinación de magia", "invisibilidad"],
        $debilidades = ["Troll", "Oscuridad"]
    ) {
    ...
    public function defender(Personaje $enemigo): void
    {
        ...
        for ($i = 0; $i < count($this->debilidades); $i++) {
            if ($this->debilidades[$i] == "Oscuridad") {
                if ($enemigo instanceof Dragon && $enemigo->getElemento() === "Oscuridad")
                    $ataque += 5;
                echo "merde me ataco un " . get_class($enemigo) . " \n";
            }
            if ($this->debilidades[$i] == "Troll") {
